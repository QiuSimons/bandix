...
- **流量配额**：按设备设置每分钟、每小时、每天、每周、每月和累计 WAN 流量上限，超限后自动断网
...
获取所有设备的流量配额、当前用量及阻断状态。配额只统计 WAN 下载与上传之和；分钟、小时、天、周、月按路由器本地时间重置（每周从周一开始），累计总量不自动重置。

#### POST /api/traffic/quotas

新增或更新设备配额。单位均为字节，`0` 表示该周期不限制。首次为设备创建配额后开始累计用量；更新配额会保留已有用量。
...
也可使用 `PUT` 提交相同请求。达到任一非零上限后，设备的 IPv4/IPv6 WAN 上传和下载都会被阻断；LAN 通信不受影响。配额通过 TC eBPF 中的 WAN 累计字节绝对阈值逐包检查，不再等待用户态的一秒采样周期。跨过阈值的当前数据包会放行，后续数据包立即丢弃，因此通常只会超出约一个数据包；多队列并发处理时可能是少量同时处理的数据包。

#### DELETE /api/traffic/quotas

删除设备配额及其累计用量，设备会在下一次监控周期（通常 1 秒内）解除配额阻断。
